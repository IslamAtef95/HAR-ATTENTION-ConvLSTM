...
class RCNN(nn.Module):
    ...
    def forward(self, input):
        ...
        # lstm_input = self.relu4(self.conv2DLayer4(self.relu3(self.conv2DLayer3
        #                                 (self.relu2(self.conv2DLayer2(self.relu1(self.conv2DLayer1(input)))))))).permute(2,0,1,3)
        # reshape to put them in the lstm
        # lstm_input = convout4.permute(2,0,1,3)
        lstm_input = input.permute(2,0,1,3)
...
def train_model(model,output_dir,num_epochs= 5, lr=1e-3, lr_decay = 0.97, momentum = 0.9, print_every = 40, evaluate_every=2, handle=None):
    import copy
        ...
        if (epoch + 1) % evaluate_every == 0:
            ...
            if score > best_score:
                ...
                torch.save(model.state_dict(), os.path.join(output_dir, str(best_score) + ".." + str(model.attention_dropout_val) + ".."
                                                                 + str(model.dropout_val) + ".." + str(lr_decay) + ".." + str(model.hidden_size)))
...
def read_cmd_args():
    ...
    model = RCNN(input_size=NB_SENSOR_CHANNELS, hidden_size=hunits,
                 num_layers=NUM_LAYERS_LSTM, is_bidirectional=True, dropout=d, attention_dropout=ad)
    if use_cuda:
        model = model.cuda()
    best_score, best_model = train_model(model,out_dir, lr_decay=lr_decay, num_epochs=60)
