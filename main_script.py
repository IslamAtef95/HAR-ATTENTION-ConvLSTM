...
def train_model(model, output_dir, process_id, num_epochs= 5, lr=1e-3, lr_decay = 0.97, momentum = 0.9, print_every = 80, evaluate_every=2, handle=None):
    import copy
        ...
        if (epoch + 1) % evaluate_every == 0:
            ...
            if score > best_score:
                ...
                torch.save(model.state_dict(), os.path.join(output_dir, str(best_score) + ".." + str(model.attention_dropout_val) + ".."
                                                                 + str(model.dropout_val) + ".." + str(lr_decay) + str(process_id)))
...
def read_cmd_args():
    ...
    best_score, best_model = train_model(model, out_dir, id, lr_decay=lr_decay, num_epochs=110)
