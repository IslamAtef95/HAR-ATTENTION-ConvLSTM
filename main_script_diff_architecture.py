...
class RCNN(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1, is_bidirectional=False, dropout=0.5,attention_dropout=0.5):
        ...
        self.conv2DLayer1 = nn.Conv2d(1, 128, (FILTER_SIZE, 1), stride=(FILTER_STRIDE,1))
        self.relu1 = nn.ReLU()
        self.conv2DLayer2 = nn.Conv2d(128, 64, (FILTER_SIZE, 1), stride=(FILTER_STRIDE,1))
        self.relu2 = nn.ReLU()
        self.conv2DLayer3 = nn.Conv2d(64, 32, (FILTER_SIZE, 1), stride=(FILTER_STRIDE,1))
        self.relu3 = nn.ReLU()
        self.conv2DLayer4 = nn.Conv2d(32, 16, (FILTER_SIZE, 1), stride=(FILTER_STRIDE,1))
        self.relu4 = nn.ReLU()
        ...
        self.attentionLayer1 = nn.Linear(input_size, input_size)
        self.tanh1 = nn.Tanh()
        self.attentionLayer2 = nn.Linear(input_size,1)
    ...
    def forward(self, input, vis_attention=False):
        ...
        # attention stuff
        attention_layer1_output = self.attentionLayer1(lstm_input)
        ...
        new_lstm_input = attn_weights * lstm_input
        output, hidden = self.lstm(new_lstm_input, self.initHidden())
        current = output[-1]
...
def train_model(model, num_epochs= 5, lr=1e-3, lr_decay = 0.97, momentum = 0.9, print_every = 80, evaluate_every=2, handle=None):
    import copy
...
def read_cmd_args():
    ...
    best_score, best_model = train_model(model, lr_decay=lr_decay, num_epochs=40)
