...
def select_columns_opp(data):
    ...
    cols_to_select = np.array([38, 39, 40, 41, 42, 43, 44, 45, 46, 51, 52,53, 54, 55, 56, 57, 58, 59, 64, 65, 66,67, 68, 69, 70, 71, 72, 77, 78,79, 80, 81, 82, 83, 84, 85,
...
def generate_data(dataset, target_filename_path, label):
    """Function to read the OPPORTUNITY challenge raw data and process all sensor channels

    :param dataset: string
        Path with original OPPORTUNITY directory
    ...
    for filename in OPPORTUNITY_DATA_FILES:
        try:
            data_arr = pd.read_table(os.path.join(dataset,filename), sep=' ', header=None, dtype=float)
    ...
    f = open(target_filename_path, 'wb')
    cp.dump(obj, f)
    f.close()
